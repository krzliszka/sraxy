...
function RSVPSection() {
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', guests: '1', attending: '', diet: '', transport: '', comments: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('success');
      setFormData({ name: '', email: '', phone: '', guests: '1', attending: '', diet: '', transport: '', comments: '' });
    } catch (error) {
      setStatus('error');
    ...
    <section id="rsvp" className="py-20 px-4 scroll-mt-20">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-2 mb-6">
            <Sparkles className="w-4 h-4 text-chocolate/50" />
            <span className="text-chocolate/70 text-sm tracking-[0.3em] uppercase font-serif">Potwierdź obecność</span>
            <Sparkles className="w-4 h-4 text-chocolate/50" />
          </div>
          <h2 className="font-handwriting text-5xl sm:text-6xl text-cranberry mb-6">RSVP</h2>
          <p className="font-serif text-xl text-graphite/80 leading-relaxed">
            Prosimy o potwierdzenie przybycia do dnia <span className="font-medium text-cranberry">1 marca 2026</span>
          </p>
        </div>

        {status === 'success' ? (
          <div className="text-center py-12 bg-sage/20 rounded-3xl">
            <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-sage/30 flex items-center justify-center">
              <CheckCircle className="w-10 h-10 text-sage" />
            </div>
            <h3 className="font-handwriting text-4xl text-cranberry mb-4">Dziękujemy!</h3>
            <p className="font-serif text-xl text-graphite/80 mb-2">Twoje potwierdzenie zostało wysłane.</p>
            <p className="font-serif text-graphite/60">Do zobaczenia na naszym ślubie!</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-8 bg-sage/10 rounded-3xl p-8">
            {status === 'error' && (
            ...
            <div>
              <label className="block font-serif text-graphite font-medium mb-2">Imię i nazwisko *</label>
              <input type="text" name="name" value={formData.name} onChange={handleChange} required
                className="w-full px-4 py-3 rounded-xl border border-chocolate/20 bg-cream font-serif text-graphite focus:outline-none focus:border-cranberry focus:ring-2 focus:ring-cranberry/20"
                placeholder="Jan Kowalski" />
            </div>

            <div className="grid sm:grid-cols-2 gap-6">
              <div>
                <label className="block font-serif text-graphite font-medium mb-2">Email</label>
                <input type="email" name="email" value={formData.email} onChange={handleChange}
                  className="w-full px-4 py-3 rounded-xl border border-chocolate/20 bg-cream font-serif text-graphite focus:outline-none focus:border-cranberry focus:ring-2 focus:ring-cranberry/20"
                  placeholder="jan@example.com" />
              </div>
              <div>
                <label className="block font-serif text-graphite font-medium mb-2">Telefon</label>
            ...
            <div>
              <label className="block font-serif text-graphite font-medium mb-4">Czy weźmiesz udział w naszym ślubie? *</label>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  { value: 'yes', label: 'Tak, z przyjemnością!', icon: Heart },
                  { value: 'no', label: 'Niestety, nie dam rady', icon: null },
                ].map((option) => (
                  <label key={option.value}
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                      formData.attending === option.value
                        ? option.value === 'yes' ? 'border-cranberry bg-cranberry/5' : 'border-chocolate bg-chocolate/5'
                        : 'border-chocolate/20 hover:border-chocolate/40'
                    }`}>
                    <input type="radio" name="attending" value={option.value} checked={formData.attending === option.value} onChange={handleChange} className="sr-only" />
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                      formData.attending === option.value ? (option.value === 'yes' ? 'border-cranberry' : 'border-chocolate') : 'border-chocolate/30'
                    }`}>
                      {formData.attending === option.value && <div className={`w-3 h-3 rounded-full ${option.value === 'yes' ? 'bg-cranberry' : 'bg-chocolate'}`} />}
                    </div>
                    <span className="font-serif text-graphite">{option.label}</span>
                    {option.icon && <option.icon className={`w-5 h-5 ml-auto ${formData.attending === option.value ? 'text-cranberry fill-cranberry' : 'text-chocolate/30'}`} />}
                  </label>
                ))}
              </div>
            </div>
            ...
            {formData.attending === 'yes' && (
              <>
                <div>
                  <label className="block font-serif text-graphite font-medium mb-2"><Users className="w-4 h-4 inline mr-2" />Liczba osób</label>
                  <select name="guests" value={formData.guests} onChange={handleChange}
                    className="w-full px-4 py-3 rounded-xl border border-chocolate/20 bg-cream font-serif text-graphite focus:outline-none focus:border-cranberry focus:ring-2 focus:ring-cranberry/20">
                    {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n} {n === 1 ? 'osoba' : n < 5 ? 'osoby' : 'osób'}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block font-serif text-graphite font-medium mb-2"><Utensils className="w-4 h-4 inline mr-2" />Wymagania dietetyczne</label>
                  <textarea name="diet" value={formData.diet} onChange={handleChange} rows={2}
                    className="w-full px-4 py-3 rounded-xl border border-chocolate/20 bg-cream font-serif text-graphite focus:outline-none focus:border-cranberry focus:ring-2 focus:ring-cranberry/20 resize-none"
                    placeholder="Wegetariańskie, wegańskie, bezglutenowe, alergie..." />
                </div>
                <div>
                  <label className="block font-serif text-graphite font-medium mb-2"><Car className="w-4 h-4 inline mr-2" />Transport</label>
            ...
            <div>
              <label className="block font-serif text-graphite font-medium mb-2"><MessageSquare className="w-4 h-4 inline mr-2" />Uwagi lub życzenia</label>
            ...
            <button type="submit" disabled={status === 'loading'}
              ...
              {status === 'loading' ? (
                <><div className="w-5 h-5 border-2 border-cream/30 border-t-cream rounded-full animate-spin" />Wysyłanie...</>
              ) : (
                <><Send className="w-5 h-5" />Wyślij potwierdzenie</>
              )}
            </button>
          </form>
        )}
        ...
        {/* Phone contact */}
        <div className="mt-12 text-center bg-sage/20 rounded-2xl p-8">
          <h3 className="font-handwriting text-3xl text-cranberry mb-4">Wolisz zadzwonić?</h3>
          <p className="font-serif text-graphite/70 mb-6">Możesz też potwierdzić obecność telefonicznie:</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="tel:+48504444866" className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-cream border-2 border-chocolate/30 rounded-full text-chocolate hover:bg-chocolate/10 transition-colors font-serif">Paula: 504-444-866</a>
            <a href="tel:+48792512711" className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-cream border-2 border-chocolate/30 rounded-full text-chocolate hover:bg-chocolate/10 transition-colors font-serif">Artur: 792-512-711</a>
          </div>
        </div>
      </div>
    </section>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="max-w-2xl mx-auto text-center bg-sage/20 rounded-3xl p-8">
          ...
          <div className="bg-cream rounded-2xl p-8 border-2 border-dashed border-chocolate/30">
            ...
            <p className="font-serif text-graphite/60 mb-4">Funkcja przesyłania zdjęć będzie dostępna po ślubie</p>
            <p className="font-serif text-sm text-graphite/40">#PaulaIArtur2026</p>
          </div>
        </div>
      </div>
