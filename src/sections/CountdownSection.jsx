...
function calcDiff() {
  const now = Date.now();
  const weddingTime = WEDDING.getTime();
  const isBefore = now < weddingTime;
...
export default function CountdownSection() {
  ...
  const title = t.isBefore ? 'Do ślubu pozostało' : 'Jesteśmy małżeństwem od';

  return (
    <section className="py-20 px-4 bg-sage/20">
      <div className="max-w-4xl mx-auto text-center">
        <h2 className="font-hand text-4xl sm:text-5xl text-cranberry mb-10">{title}</h2>
